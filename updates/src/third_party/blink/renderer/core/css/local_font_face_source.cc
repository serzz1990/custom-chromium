...
//START-UPDATES
  if (base::CustomDevice::GetInstance()->HasDict("fonts")) {
    if (base::CustomDevice::GetInstance()->HasFont(font_name_.Utf8())) {
//      std::cout << "font available" << font_name_ << "\n";
      font_selector_->ReportSuccessfulLocalFontMatch(font_name_);
      return true;
    } else {
//      std::cout << "font not available" << font_name_ << "\n";
